...
namespace ParticleLibrary.Debug
{
	internal class DebugUI : UIState
	{
		...
		internal void DrawInfoPanel(SpriteBatch spriteBatch)
		{
			...
			if (scaleDropdown.enabled)
			{
				DrawInfoString("Vector Scale", $"({TrackedParticle?.scale.X:N1}, {TrackedParticle?.scale.Y:N1})");
		...
		internal void DrawCountPanel(SpriteBatch spriteBatch)
		{
			...
			spriteBatch.DrawString(MouseText.Value, ParticleManager.particles.Count.ToString(), position - new Vector2(-10f, 15f), Color.White, 0f, Vector2.Zero, 0.75f, SpriteEffects.None, 0f);
		}
