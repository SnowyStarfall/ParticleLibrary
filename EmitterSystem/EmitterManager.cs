...
{
	public class EmitterManager : ModWorld
	{
		...
		public override TagCompound Save()
		{
			...
			List<EmitterSerializer> e = emitters.ConvertAll((o) => new EmitterSerializer(o));
			e.RemoveAll(x => x == null);
		...
		private static void DrawEmitters(On.Terraria.Main.orig_DrawDust orig, Main self)
		{
			if (Main.netMode != NetmodeID.Server)
			{
				...
				for (int i = 0; i < emitters?.Count; i++)
				{
					...
					if (emitter != null)
						emitter.Draw(Main.spriteBatch, emitter.position - Main.screenPosition, Lighting.GetColor((int)(emitter.position.X / 16), (int)(emitter.position.Y / 16)));
				}
		...
		public static Emitter NewEmitter(Vector2 Position, Emitter Emitter, string Data = null, float CullDistance = -1f)
		{
			...
			{
				Mod parsed =  ModLoader.GetMod(type.GetType().Assembly.GetName().Name);
				if (parsed != null)
					type.Data = parsed.Name + ": " + type.GetType().Name;
				else
