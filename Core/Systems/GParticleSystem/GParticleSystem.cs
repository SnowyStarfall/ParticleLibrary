...
namespace ParticleLibrary.Core.Systems.Test
{
	public class GParticleSystem
	{
		...
		public GParticleSystemSettings Settings { get; private set; }
		...
		private EffectParameter _terminalGravity;
		...
		public GParticleSystem(GParticleSystemSettings settings)
		{
			...
			Main.QueueMainThreadAction(() =>
			{
				...
				_vertexBuffer = new(Device, typeof(GParticleVertex), Settings.MaxParticles * 4, BufferUsage.WriteOnly);
				_indexBuffer = new(Device, IndexElementSize.ThirtyTwoBits, Settings.MaxParticles * 6, BufferUsage.WriteOnly);

				_vertices = new GParticleVertex[Settings.MaxParticles * 4];
				_indices = new int[Settings.MaxParticles * 6];
			});
		...
		protected virtual void Draw()
		{
			...
			Device.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, 0, Settings.MaxParticles * 4, 0, Settings.MaxParticles * 2);
		}

		public void AddParticle(Vector2 position, Vector2 velocity, GParticleSettings settings)
		{
			if (_currentParticleIndex >= Settings.MaxParticles)
				return;

			Vector2 size = new(Settings.Texture.Width * settings.Scale.X, Settings.Texture.Height * settings.Scale.Y);
			...
			_vertices[_currentParticleIndex * 4] = new GParticleVertex()
			{
				...
				Size = new Vector2(Settings.Texture.Width, Settings.Texture.Height),
		...
		public void SetLifespan(int value)
		{
			...
			Settings.Lifespan = value;
			_lifespan.SetValue(value);
		}

		public void SetFade(bool value)
		{
			Settings.Fade = value;
			_fade.SetValue(value);
		}

		public void SetGravity(float value)
		{
			Settings.Gravity = value;
			_gravity.SetValue(value);
		}

		public void SetTerminalGravity(float value)
		{
			Settings.TerminalGravity = value;
			_terminalGravity.SetValue(value);
		}
		...
		private void SetParameters()
		{
			...
			_terminalGravity = _effect.Parameters["TerminalGravity"];
			_texture = _effect.Parameters["Texture"];

			ResolutionChanged(Main.ScreenSize.ToVector2());
			_lifespan.SetValue(Settings.Lifespan);
			_fade.SetValue(Settings.Fade);
			_gravity.SetValue(Settings.Gravity);
			_terminalGravity.SetValue(Settings.TerminalGravity);
			_texture.SetValue(Settings.Texture);
		}
	}
}
